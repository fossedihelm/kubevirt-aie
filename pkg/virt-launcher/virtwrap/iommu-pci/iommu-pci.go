...
//
// Parameters:
//   - arch: The CPU architecture (typically "arm64", "x86_64", "s390x")
//
// Returns:
//   - *IommuPCI: Initialized IOMMU configuration object
func NewIommuPCI(arch string) *IommuPCI {
	// Only perform checks on ARM64 architecture where SMMUv3 is relevant
	if arch != "arm64" {
	...
	return &IommuPCI{
		...
		PCIHoleSize:    0, // Will be calculated when devices are analyzed
	}
}
...
//
// The BDF identifier follows the PCI naming convention, typically formatted
// as "DDDD:BB:DD.F" (Domain:Bus:Device.Function), e.g., "0000:3b:00.0".
//
// Parameters:
//   - id: PCI BDF identifier string
//
// Returns:
//   - *BDF: Initialized BDF device object (capabilities not yet populated)
